...
var Container = (function(_Widget) {
	...
	Container.prototype.init = function init(context) {
		...
		if (this.layout) {
			var layout = Widget.create({ type: this.layout, items: items });
			layout.init(context);
			this.layout = null;
			if (layout.noLayout) {
	...
	Container.prototype.exploreItems = function exploreItems(context, instance, items) {
	...
	Container.prototype.render = function render(context, instance) {
		return this.renderChildren(context, instance);
	};

	Container.prototype.renderChildren = function renderChildren(context, instance) {
	...
	Container.prototype.add = function add() {
		...
		if (this.layout) return (_layout = this.layout).add.apply(_layout, args);
		...
		args.forEach(function(a) {
			...
			} else if (a.isComponent) _this2.items.push(_this2.wrapItem(a));
			else {
				_this2.add(Widget.create(a, _this2.itemDefaults));
			}
		});
	};

	Container.prototype.wrapItem = function wrapItem(item) {
		return item;
	};
...
var UseParentLayout = (function(_PureContainer) {
	inherits(UseParentLayout, _PureContainer);
...
var Instance = (function() {
	...
	Instance.prototype.getDetachedChild = function getDetachedChild(widget, key, store) {
...
var Cx = (function(_VDOM$Component) {
	...
	function Cx(props) {
		...
		if (props.instance) {
			...
			_this.widget = PureContainer.create({ items: props.widget || props.items });
	...
	Cx.prototype.getInstance = function getInstance() {
...
var RestateStore = (function(_Store) {
	...
	function RestateStore(config) {
		...
		var _this2 = possibleConstructorReturn(this, _Store.call(this, config));
	...
	RestateStore.prototype.setParentData = function setParentData(data) {
		...
		var changed = this.silently(function() {
			for (var key in data) {
				_Store.prototype.setItem.call(_this3, key, data[key]);
			}
		});
	...
	RestateStore.prototype.doNotify = function doNotify(path) {
		var _this4 = this;
		...
		var changed = this.store.batch(function() {
			...
			for (var key in _this4.bindings) {
				...
				if (value !== _this4.parentData[key]) {
...
var DataProxy = (function(_PureContainer) {
	inherits(DataProxy, _PureContainer);

	function DataProxy() {
		classCallCheck(this, DataProxy);
		return possibleConstructorReturn(this, _PureContainer.apply(this, arguments));
	}

	DataProxy.prototype.init = function init() {
	...
	DataProxy.prototype.initInstance = function initInstance(context, instance) {
		var _this2 = this;
...
var DataProxyView = (function(_ReadOnlyDataView) {
	inherits(DataProxyView, _ReadOnlyDataView);

	function DataProxyView(config) {
		classCallCheck(this, DataProxyView);

		var _this3 = possibleConstructorReturn(this, _ReadOnlyDataView.call(this, config));

		_this3.dataSelector = getSelector(_this3.privateData);
		if (_this3.dataSelector.memoize) _this3.dataSelector = _this3.dataSelector.memoize();
		return _this3;
	}

	DataProxyView.prototype.getAdditionalData = function getAdditionalData(parentStoreData) {
		if (this.meta.version !== this.cache.version) this.data = this.dataSelector(parentStoreData);
		return this.data;
	};

	DataProxyView.prototype.setItem = function setItem(path, value) {
	...
	DataProxyView.prototype.deleteItem = function deleteItem(path) {
...
var FunctionalComponent = (function(_PureContainer) {
	inherits(FunctionalComponent, _PureContainer);

	function FunctionalComponent() {
		classCallCheck(this, FunctionalComponent);
		return possibleConstructorReturn(this, _PureContainer.apply(this, arguments));
	}

	FunctionalComponent.prototype.initInstance = function initInstance(context, instance) {
		this.clear();
		this.add(
	...
	FunctionalComponent.prototype.explore = function explore(context, instance) {
		if (this.layout) this.layout.items = instance.content;
		else this.items = instance.content;
...
function createFunctionalComponent(factory) {
	...
	return createComponentFactory(factory, function() {
...
var FirstVisibleChildItem = (function(_PureContainer) {
	inherits(FirstVisibleChildItem, _PureContainer);

	function FirstVisibleChildItem() {
		classCallCheck(this, FirstVisibleChildItem);
		return possibleConstructorReturn(this, _PureContainer.apply(this, arguments));
	}

	FirstVisibleChildItem.prototype.checkVisible = function checkVisible(context, instance, data) {
	...
	FirstVisibleChildItem.prototype.explore = function explore(context, instance) {
		if (instance.parent.firstVisibleChild) return;
		_PureContainer.prototype.explore.call(this, context, instance);
	};

	FirstVisibleChildItem.prototype.exploreCleanup = function exploreCleanup(context, instance) {
		if (instance.parent.firstVisibleChild) return;
		if (isVisibleDeep(instance)) instance.parent.firstVisibleChild = instance;
	};

	FirstVisibleChildItem.prototype.render = function render(context, instance, key) {
...
var FirstVisibleChildLayout = (function(_PureContainer2) {
	inherits(FirstVisibleChildLayout, _PureContainer2);
	...
	FirstVisibleChildLayout.prototype.explore = function explore(context, instance) {
	...
	FirstVisibleChildLayout.prototype.exploreCleanup = function exploreCleanup(context, instance) {
	...
	FirstVisibleChildLayout.prototype.wrapItem = function wrapItem(item) {
...
var LabelsLeftLayout = (function(_PureContainer) {
	inherits(LabelsLeftLayout, _PureContainer);
	...
	LabelsLeftLayout.prototype.render = function render(context, instance, key) {
		var result = [];
		var children = instance.children;
		var CSS = this.CSS,
		...
		return VDOM$1.createElement(
			"table",
			{ key: key, className: CSS.block(baseClass, this.mod) },
...
var LabelsTopLayout = (function(_PureContainer) {
	inherits(LabelsTopLayout, _PureContainer);
	...
	LabelsTopLayout.prototype.render = function render(context, instance, key) {
		var _this2 = this;
...
var TreeAdapter = (function(_ArrayAdapter) {
	...
	TreeAdapter.prototype.sort = function sort(sorters) {
		if (this.foldersFirst) {
			if (!sorters || !isArray(sorters)) sorters = [];

			sorters = [{ field: this.leafField, direction: "ASC" }].concat(sorters);
		}
