...
var Section = (function(_Container) {
	...
	Section.prototype.add = function add(item) {
		...
		if (item && item.putInto == "header")
			...
			(_Container$prototype$ = _Container.prototype.add).call.apply(
				_Container$prototype$,
				[this].concat(Array.prototype.slice.call(arguments))
			);
	};
...
var FlexBox = (function(_Container) {
	inherits(FlexBox, _Container);
	...
	FlexBox.prototype.prepareCSS = function prepareCSS(context, instance) {
		...
		data.stateMods = _extends(
			...
			(_babelHelpers$extends.nested = this.nested),
			_babelHelpers$extends)
		);
...
var Resizer = (function(_Widget) {
	inherits(Resizer, _Widget);
	...
	Resizer.prototype.declareData = function declareData() {
		var _Widget$prototype$dec;
	...
	Resizer.prototype.render = function render(context, instance, key) {
		var data = instance.data;
...
var ResizerCmp = (function(_VDOM$Component) {
	inherits(ResizerCmp, _VDOM$Component);

	function ResizerCmp(props) {
		classCallCheck(this, ResizerCmp);
	...
	ResizerCmp.prototype.shouldComponentUpdate = function shouldComponentUpdate(props, state) {
		return state != this.state;
	};

	ResizerCmp.prototype.render = function render() {
		var _this3 = this;
		...
		return VDOM.createElement(
			"div",
			{
				...
				onMouseDown: function onMouseDown(e) {
					...
					_this3.setState({ dragged: true, initialPosition: initialPosition });
				},
	...
	ResizerCmp.prototype.startCapture = function startCapture(e) {
		var instance = this.props.instance;
		var widget = instance.widget;
	...
	ResizerCmp.prototype.onHandleMove = function onHandleMove(e, initialPosition) {
		var instance = this.props.instance;
		var widget = instance.widget;
	...
	ResizerCmp.prototype.getNewSize = function getNewSize(offset) {
		var _props2 = this.props,
			instance = _props2.instance,
			data = _props2.data;
	...
	ResizerCmp.prototype.onDragComplete = function onDragComplete() {
...
var Overlay = (function(_Container) {
	inherits(Overlay, _Container);
...
var Tooltip = (function(_Dropdown) {
	...
	Tooltip.prototype.overlayDidMount = function overlayDidMount(instance, component) {
		...
		instance.parentValidityCheckTimer = setInterval(function() {
			if (!_this3.relatedElement.ownerDocument.body.contains(_this3.relatedElement)) {
...
var Window = (function(_Overlay) {
	...
	Window.prototype.handleKeyDown = function handleKeyDown(event, instance, component) {
...
var MenuItemComponent$1 = (function(_VDOM$Component) {
	...
	MenuItemComponent.prototype.render = function render() {
		...
		var dropdown =
			...
			VDOM.createElement(Cx, { widget: this.getDropdown(), options: { name: "submenu" }, parentInstance: instance });
		...
		if (empty) children = VDOM.createElement("span", { className: CSS.element(baseClass, "baseline") }, "\xA0");
	...
	MenuItemComponent.prototype.componentDidMount = function componentDidMount() {
		...
		if (widget.keyboardShortcut)
			this.unregisterKeyboardShortcut = registerKeyboardShortcut(widget.keyboardShortcut, function(e) {
				_this5.el.focus(); //open the dropdown
				_this5.onClick(e); //execute the onClick handler
			});
			...
			if (widget.hoverToOpen) FocusManager.focus(this.el);
				...
				if (commonParentMenu)
					this.autoFocusTimerId = setTimeout(function() {
						delete _this6.autoFocusTimerId;
						if (!_this6.state.dropdownOpen) {
							debug(menuFlag, "MenuItem", "hoverFocusTimeout:before", _this6.el);
							FocusManager.focus(_this6.el);
							debug(menuFlag, "MenuItem", "hoverFocusTimeout:after", _this6.el, getActiveElement());
						}
		...
		if (widget.dropdown) {
			if (
				...
				this.openDropdown(function() {
					var focusableChild = findFirstChild(_this7.el, isFocusable);
...
var MenuComponent = (function(_VDOM$Component) {
			...
			children.map(function(c, i) {
				...
				return VDOM.createElement(
					MenuItemComponent,
					{
						...
						hidden: i >= _this3.state.visibleItemCount && i + 1 != children.length,
						instance: instance,
	...
	MenuComponent.prototype.componentDidMount = function componentDidMount() {
		...
		if (widget.overflow) this.unsubscribeResize = ResizeManager.trackElement(this.el, this.measureOverflow.bind(this));
	};

	MenuComponent.prototype.componentDidUpdate = function componentDidUpdate() {
		this.measureOverflow();
	};

	MenuComponent.prototype.measureOverflow = function measureOverflow() {
		var instance = this.props.instance;
		var widget = instance.widget;
...
var SelectComponent = (function(_VDOM$Component) {
	...
	SelectComponent.prototype.onKeyDown = function onKeyDown(e) {
		switch (e.keyCode) {
			case KeyCode.up:
			case KeyCode.down:
				e.stopPropagation();
...
var LookupComponent = (function(_VDOM$Component) {
	...
	LookupComponent.prototype.onInputKeyDown = function onInputKeyDown(e) {
		var instance = this.props.instance;
...
var NumberField = (function(_Field) {
	...
	NumberField.prototype.validateRequired = function validateRequired(context, instance) {
...
var Input$1 = (function(_VDOM$Component) {
	...
	Input.prototype.onChange = function onChange(e, change) {
		...
		if (change == "blur" || change == "enter") instance.setState({ visited: true });
...
var GridRowComponent = (function(_VDOM$Component) {
	...
	function GridRowComponent(props) {
		...
		if (grid.widget.cellEditable)
			_this2.onDoubleClick = function(e) {
				_this2.props.parent.moveCursor(_this2.props.cursorIndex, { cellEdit: true });
	...
	GridRowComponent.prototype.getCellIndex = function getCellIndex(e) {
...
var GridComponent = (function(_VDOM$Component) {
	...
	GridComponent.prototype.render = function render() {
		...
		var _state = this.state,
			...
			cursorCellIndex = _state.cursorCellIndex,
			cellEdit = _state.cellEdit;
							...
							content.map(function(_ref3, cellIndex) {
								...
								if (cellected && cellEdit) {
									...
									if (column && column.editor && data.editable)
										...
										return VDOM.createElement(
											"td",
											{ key: key, className: CSS.element(baseClass, "cell-editor") },
											VDOM.createElement(
												"div",
												{
													className: CSS.element(baseClass, "cell-editor-wrap"),
													style: _this6.rowHeight > 0 ? { height: _this6.rowHeight + 1 } : null
												},
													...
													items: [
														{
															...
															valid: {
																get: function get$$1() {
																	return _this6.cellEditorValid;
																},
																set: function set$$1(value) {
																	_this6.cellEditorValid = value;
																}
															},
															jsxAttributes: ["valid"],
															children: [
		...
		if (widget.buffered) {
			...
			var dummyDataClass = CSS.element(baseClass, "data", { dummy: true });
					...
					if (s >= offset && e <= offset + records.length) {
						...
						var result = instance.invoke(
							"onFetchRecords",
							{
								...
								state: _this7.prevFetchRecordsState
							},
							instance
						);
					...
					.then(function(pageRecords) {
						...
						pageRecords.forEach(function(page) {
							if (Array.isArray(page)) {
								...
								_this7.prevFetchRecordsState = page.state;
		...
		if (Object.keys(newState).length > 0) {
			...
			this.setState(newState, function() {
				...
				if (!_this9.state.cellEdit && wasCellEditing) {
					FocusManager.focus(_this9.dom.scroller);
				...
				if (scrollIntoView) {
					...
					if (widget.cellEditable && _this9.state.cursorCellIndex >= 0 && item)
						item = item.firstChild.children[_this9.state.cursorCellIndex];
	...
	GridComponent.prototype.getRecordAt = function getRecordAt(cursor) {
		var _props6 = this.props,
			instance = _props6.instance,
			data = _props6.data;
		var records = instance.records,
	...
	GridComponent.prototype.getRecordInstanceAt = function getRecordInstanceAt(cursor) {
...
var DragSource = (function(_Container) {
	inherits(DragSource, _Container);
...
var DragHandle = (function(_Container) {
	inherits(DragHandle, _Container);
...
var DropZone = (function(_Container) {
	inherits(DropZone, _Container);
