...
var Ref = (function(_Component) {
	...
	Ref.prototype.get = function get$$1() {
		return this.store.get(this.path);
	};

	Ref.prototype.set = function set$$1(value) {
		return this.store.set(this.path, value);
	};

	Ref.prototype.init = function init(value) {
		return this.store.init(this.path, value);
	};

	Ref.prototype.toggle = function toggle() {
		return this.store.toggle(this.path);
	};

	Ref.prototype.delete = function _delete() {
		return this.store.delete(this.path);
	};

	Ref.prototype.update = function update() {
		var _store;
		...
		return (_store = this.store).update.apply(_store, [this.path].concat(args));
	};

	Ref.prototype.as = function as(config) {
	...
	Ref.prototype.memoize = function memoize() {
		return this.get;
	};
...
Ref.factory = function(alias, config, more) {
	if (isFunction(alias)) {
		...
		return Ref.create(_extends({}, config, more, result));
	}

	return Ref.create(_extends({}, config, more));
};
...
var View = (function() {
	...
	View.prototype.ref = function ref(path, defaultValue) {
	...
	View.prototype.getMethods = function getMethods() {
		return {
...
var ReadOnlyDataView = (function(_View) {
	...
	ReadOnlyDataView.prototype.getAdditionalData = function getAdditionalData() {
		return this.data;
	};
...
var ArrayRef = (function(_Ref) {
	inherits(ArrayRef, _Ref);

	function ArrayRef() {
		classCallCheck(this, ArrayRef);
		return possibleConstructorReturn(this, _Ref.apply(this, arguments));
	}

	ArrayRef.prototype.append = function append$$1() {
		for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
			args[_key] = arguments[_key];
		}
	...
	ArrayRef.prototype.insert = function insert(index) {
	...
	ArrayRef.prototype.filter = function filter(predicate) {
	...
	ArrayRef.prototype.move = function move(fromIndex, toIndex) {
		this.update(moveElement, fromIndex, toIndex);
	};

	ArrayRef.prototype.clear = function clear() {
		this.set([]);
	};

	ArrayRef.prototype.sort = function sort(compare) {
		var data = this.get();
		if (!data) return;
		var x = [].concat(data);
		x.sort(compare);
...
var StoreProxy = (function(_View) {
	inherits(StoreProxy, _View);

	function StoreProxy(getStore) {
		classCallCheck(this, StoreProxy);
	...
	StoreProxy.prototype.getData = function getData() {
